...
fn render_text(
    ...
    for entry in &sorted {
        let (symbol, kind_label, color_fn): (
            &str,
            &str,
            fn(&str) -> ColoredString,
        ) = match entry.kind {
...
fn render_json(
    ...
    for entry in &result.entries {
        ...
        if let Some(ref old) = entry.old_value {
            obj.insert("old_value".to_string(), serde_json::Value::String(old.clone()));
        }
        if let Some(ref new) = entry.new_value {
            obj.insert("new_value".to_string(), serde_json::Value::String(new.clone()));
        }
